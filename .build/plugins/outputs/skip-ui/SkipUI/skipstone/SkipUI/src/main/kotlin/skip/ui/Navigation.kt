            ...
            MaterialTheme(colorScheme = materialColorScheme) { ->
                ...
                EnvironmentValues.shared.setValues({ it ->
                    ...
                    PaddingLayout(padding = EdgeInsets(top = 0.0, leading = 0.0, bottom = Double(-bottomPadding.value), trailing = 0.0), context = context.content()) { context ->
                        BottomAppBar(modifier = context.modifier.then(bottomBarModifier), containerColor = bottomBarBackgroundColor, contentPadding = PaddingValues.Absolute(left = 16.dp, right = 16.dp), windowInsets = WindowInsets(bottom = 0.dp)) { ->
...
internal class Navigator {
    ...
    private fun navigateToPath() {
        ...
        for (unusedbinding in 0..<(backStack.count() - backStackIndex)) {
            navController.popBackStack()
        }
...
class NavigationLink: View, ListItemAdapting {
    ...
    @Composable
    override fun ComposeContent(context: ComposeContext): Unit = Button.ComposeButton(label = label, context = context, isEnabled = isNavigationEnabled(), action = navigationAction())
    ...
    override fun ComposeListItem(context: ComposeContext, contentModifier: Modifier) {
        val isEnabled = isNavigationEnabled()
        val modifier = Modifier.clickable(onClick = navigationAction(), enabled = isEnabled).then(contentModifier)
    ...
    @Composable
    private fun isNavigationEnabled(): Boolean = (value != null || destination != null) && EnvironmentValues.shared.isEnabled

    @Composable
    private fun navigationAction(): () -> Unit {
