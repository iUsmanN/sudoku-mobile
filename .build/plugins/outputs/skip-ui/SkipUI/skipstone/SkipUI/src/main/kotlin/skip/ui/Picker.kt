...
class Picker<SelectionValue>: View, ListItemAdapting {
    ...
    private fun ComposeSelectedValue(views: Array<TagModifierView>, context: ComposeContext, style: PickerStyle, performsAction: Boolean = true) {
        ...
        if (performsAction) {
            ...
            var foregroundStyle = (EnvironmentValues.shared._tint ?: Color(colorImpl = { -> androidx.compose.ui.graphics.Color.Gray })).sref()
                    ...
                    HStack { ->
                        ComposeBuilder { composectx: ComposeContext ->
                            ...
                            Image(systemName = "checkmark")
                                ...
                                .opacity(if (label.value == selection.wrappedValue) 1.0 else 0.0).Compose(composectx)
                            ComposeResult.ok
                        }
                    }.Compose(composectx)
                    ComposeResult.ok
