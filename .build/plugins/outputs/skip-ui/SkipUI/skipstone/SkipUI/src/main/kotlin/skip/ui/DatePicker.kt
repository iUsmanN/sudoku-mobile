...
class DatePicker: View {
    ...
    private fun ComposePickerContent(context: ComposeContext) {
        ...
        dateFormatter?.string(from = date)?.let { dateString ->
            ...
            if (isEnabled) {
                Button.ComposeTextButton(label = text, context = context) { -> isDatePickerPresented.value = true }
            } else {
        ...
        timeFormatter?.string(from = date)?.let { timeString ->
            ...
            if (isEnabled) {
                Button.ComposeTextButton(label = text, context = context) { -> isTimePickerPresented.value = true }
            } else {
                text.Compose(context = context)
            }
        }
