    ...
    override fun appendLazyItemViews(to: MutableList<View>, appendingContext: ComposeContext): ComposeResult {
        ...
        if (indexRange != null) {
            for (index in indexRange) {
                ...
                identifier?.let { identifier ->
                    contentViews = taggedViews(for_ = contentViews, defaultTag = index, context = appendingContext)
                }
                contentViews.forEach { it -> it.Compose(appendingContext) }
            }
        } else if (objects != null) {
            for (object_ in objects.sref()) {
                var contentViews = collectViews(from = objectContent!!(object_), context = appendingContext)
                if (!isUnrollRequired(contentViews = contentViews, isFirstView = isFirstView, context = appendingContext)) {
                    views.add(this)
                    return ComposeResult.ok
                } else {
                ...
                identifier?.let { identifier ->
                    contentViews = taggedViews(for_ = contentViews, defaultTag = identifier(object_), context = appendingContext)
                }
                contentViews.forEach { it -> it.Compose(appendingContext) }
            }
        } else if (objectsBinding != null) {
            ...
            for (i in 0..<objects.count) {
                ...
                identifier?.let { identifier ->
                    contentViews = taggedViews(for_ = contentViews, defaultTag = identifier(objects[i]), context = appendingContext)
                }
                contentViews.forEach { it -> it.Compose(appendingContext) }
            }
        }
