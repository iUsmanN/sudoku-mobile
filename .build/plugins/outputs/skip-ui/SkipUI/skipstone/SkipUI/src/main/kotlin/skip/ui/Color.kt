...
class Color: ShapeStyle, Sendable {
    ...
    constructor(hue: Double, saturation: Double, brightness: Double, opacity: Double = 1.0, @Suppress("UNUSED_PARAMETER") unusedp_0: Nothing? = null) {
        colorImpl = { -> androidx.compose.ui.graphics.Color.hsl(hue = Companion.clamp(hue), saturation = Companion.clamp(saturation), lightness = Companion.clamp(brightness), alpha = Companion.clamp(opacity)) }
    }
    ...
    companion object: ShapeStyleCompanion {
        ...
        internal var separator: Color = Color(colorImpl = { -> MaterialTheme.colorScheme.surfaceVariant })
