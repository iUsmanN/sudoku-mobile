...
class Label: View {
    ...
    @Composable
    private fun ComposeLabel(context: ComposeContext, imageColor: Color? = null, titlePadding: Double = 0.0) {
        Row(modifier = context.modifier, horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) { ->
            if (imageColor != null) {
                EnvironmentValues.shared.setValues({ it -> it.set_foregroundStyle(imageColor) }, in_ = { -> image.Compose(context = context.content()) })
            } else {
